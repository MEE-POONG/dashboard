  ...
  const navigationItems: MenuItem[] = [
    ...
    return (
        <>
            ...
            <aside className={`w-60 md:w-60 ${isMaxSidebar ? '' : '-translate-x-48'} fixed transition transform ease-in-out duration-1000 z-50 flex h-screen bg-[#1E293B]`}>
                {/* <!-- open sidebar button --> */}
                <div className="max-toolbar translate-x-24 scale-x-0 w-full -right-6 transition transform ease-in duration-300 flex items-center justify-between border-4 border-white dark:border-[#0F172A] bg-[#1E293B]  absolute top-2 rounded-full h-12">
                    ...
                    <div className="flex items-center space-x-3 group bg-gradient-to-r dark:from-cyan-500 dark:to-blue-500 from-indigo-500 via-purple-500 to-purple-500  pl-4 pr-1 py-1 rounded-full text-white">
                        <div className="transform ease-in-out duration-300 mr-11 md:mr-12 font-bold text-sm md:text-base">
                            MNR - Dashboard
                        </div>
                    </div>
                </div>
                <div onClick={openNav} className="-right-7 md:-right-5 transition transform ease-in-out duration-500 flex border-4 border-white dark:border-[#0F172A] bg-[#1E293B] dark:hover:bg-blue-500 hover:bg-purple-500 absolute top-2 p-3 rounded-full text-white hover:rotate-45">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={3} stroke="currentColor" className="w-4 h-4">
                ...
                <div className="max hidden text-white mt-20 flex-col space-y-2 w-full h-[calc(100vh)] text-sm md:text-base">
                    {navigationItems.map((item, index) => (
                        <div
                            key={index}
                            className="hover:ml-4 w-full text-white hover:text-purple-500 dark:hover:text-blue-500 bg-[#1E293B] p-2 pl-4 rounded-full transform ease-in-out duration-300 flex flex-row items-center space-x-3">
                            {item.icon}
                            <Link href={item.href}>
                                {item.text}
                            </Link>
